...
class OpenAQDataFetcher:
    ...
    def download_data_from_s3(self, location_id: str, start_date: str, end_date: str) -> bool:
        ...
        try:
            ...
            logger.info(f"总共下载了 {len(downloaded_files)} 个文件，跳过了 {skipped_nonexistent_count} 个已知不存在的文件")
    ...
    def process_with_openaq_processor(self, df: pd.DataFrame) -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame], Optional[OpenAQProcessor]]:
        """
